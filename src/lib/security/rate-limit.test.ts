...
describe('memory rate limiter', () => {
  it('allows up to the window max then blocks with a retry hint', async () => {
    let now = 1_000_000;
    const rl = new MemoryRateLimiter({ windowSeconds: 900, windowMax: 2, daySeconds: 86400, dayMax: 5 }, () => now);
  ...
  it('enforces the daily cap independently', async () => {
    let now = 0;
    const rl = new MemoryRateLimiter({ windowSeconds: 1, windowMax: 10, daySeconds: 86400, dayMax: 3 }, () => now);
    for (let i = 0; i < 3; i++) {
  ...
  it('does not block other keys', async () => {
    const rl = new MemoryRateLimiter({ windowSeconds: 900, windowMax: 1, daySeconds: 86400, dayMax: 5 });
...
describe('client ip', () => {
  it('prefers x-real-ip then the first forwarded hop', () => {
    expect(clientIp(new Headers({ 'x-real-ip': '1.1.1.1', 'x-forwarded-for': '2.2.2.2, 3.3.3.3' }))).toBe('1.1.1.1');
