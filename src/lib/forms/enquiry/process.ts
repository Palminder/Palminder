import 'server-only';
import { fieldErrorsFromZod, formDataToEnquiryInput, enquirySchema, type FieldErrors } from './schema';
...
export type ProcessResult =
  ...
  | { status: 'rejected'; httpStatus: 403 | 413 | 415 | 429; message: string; retryAfterSeconds?: number }
...
export async function processEnquiry(request: Request): Promise<ProcessResult> {
  ...
  if (length > MAX_REQUEST_BYTES) {
    return { status: 'rejected', httpStatus: 413, message: 'The request is too large. Attachments are limited to 8 MB.' };
  }

  // 3. Origin / CSRF expectations.
  if (!isTrustedOrigin(request.headers, siteUrl())) {
    return { status: 'rejected', httpStatus: 403, message: 'This request did not come from the website.' };
  }
  ...
  const tooFast = Number.isFinite(startedAt) && startedAt > 0 && Date.now() - startedAt < MIN_SUBMISSION_INTERVAL_MS;
  ...
  try {
    ...
    return { status: 'unavailable', message: 'The enquiry form is temporarily unavailable. Please email us instead.' };
  }
  if (!decision.allowed) {
    return {
      status: 'rejected',
      httpStatus: 429,
      message: 'Too many enquiries have been sent from this connection. Please try again later or email us directly.',
  ...
  if (file) {
    if (!uploadsEnabled()) {
      fieldErrors.file = 'File uploads are not available at the moment. Please send the enquiry without a file.';
    } else {
      ...
      if (!check.ok) {
        ...
        if (!scan.clean) {
          ...
          attachment = { filename: check.storedName, content: check.bytes, contentType: check.mime };
  ...
  if (!parsed.success || Object.keys(fieldErrors).length > 0) {
    return { status: 'invalid', fieldErrors, message: 'Some details need attention before the enquiry can be sent.' };
  }

  // 10. Idempotency key.
  const supplied = data.get(SUBMISSION_ID_FIELD);
  const submissionId = typeof supplied === 'string' && UUID.test(supplied) ? supplied.toLowerCase() : crypto.randomUUID();
  ...
  const sent = await sendEnquiryEmails({ enquiry: parsed.data, submissionId, attachment, attachmentOriginalName: originalName });
  if (!sent.ok) {
    console.error('[enquiry] send failed', { submissionId, reason: sent.reason });
    return { status: 'unavailable', message: 'Your enquiry could not be sent just now. Please email us directly.' };
  }
  console.info('[enquiry] sent', { submissionId, projectType: parsed.data.projectType, hasAttachment: Boolean(attachment) });
  return { status: 'success', submissionId };
}
