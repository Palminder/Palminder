...
export function PageHero({
  ...
  return (
    <section
      className={cn('container-site pt-10 pb-12 md:pt-14 md:pb-16 xl:pt-20 xl:pb-24', className)}
    >
      <div className="grid-site items-end">
        <div
          className={cn(
            'col-span-4 md:col-span-8',
            image ? 'xl:col-span-5' : narrow ? 'xl:col-span-8' : 'xl:col-span-10',
          )}
        >
          ...
          {lead ? (
            <div className="type-lead reveal reveal-delay measure mt-6 text-ink/85">{lead}</div>
          ) : null}
          {children ? (
            <div className="reveal reveal-delay mt-8 flex flex-wrap items-center gap-4">
              {children}
            </div>
          ) : null}
        </div>
