...
export function EditorialIntro({ eyebrow, heading, children, link, as: Tag = 'h2', className, layout = 'split' }: EditorialIntroProps) {
  if (layout === 'stacked') {
    return (
      <div className={cn('max-w-[62rem]', className)}>
        ...
        <div className="measure mt-6 space-y-5 text-[1.125rem] leading-[1.6] text-ink/85">{children}</div>
  ...
  return (
    <div className={cn('grid-site', className)}>
      ...
      <div className="col-span-4 mt-6 md:col-span-8 xl:col-span-6 xl:col-start-7 xl:mt-2">
        <div className="measure space-y-5 text-[1.125rem] leading-[1.6] text-ink/85">{children}</div>
        {link ? <div className="mt-8">{link}</div> : null}
      </div>
    </div>
  );
