...
export const project = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'string',
      group: 'content',
      description: 'e.g. “Kelvinside, Glasgow”. Never a full private address.',
      validation: (rule) => rule.required(),
    }),
    defineField({
      name: 'area',
    ...
    defineField({
      name: 'gallery',
      type: 'array',
      of: [defineArrayMember({ type: 'imageWithMeta' })],
      group: 'media',
    }),
    defineField({
      name: 'drawings',
  ...
  validation: (rule) =>
    rule.custom((doc) => {
      const d = doc as
        { realityType?: string; verificationStatus?: string; status?: string } | undefined;
      if (!d) return true;
